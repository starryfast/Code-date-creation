...
#define IDC_BUTTON_DEFAULT_COLOR   142   // 新增：设为默认颜色
...
void SwitchMode(HWND hWnd, int mode) {
    g_currentMode = mode;
    int randomCtrls[] = {
        IDC_EDIT_START, IDC_EDIT_END, IDC_COMBO_TYPE, IDC_EDIT_RANGE1, IDC_EDIT_RANGE2, IDC_EDIT_STRLEN,
    ...
    int manualCtrls[] = {
        IDC_EDIT_MANUAL_INNAME, IDC_EDIT_MANUAL_INCONTENT, IDC_BUTTON_MANUAL_SAVEIN,
        IDC_STATIC_MANUAL_LABEL1, IDC_STATIC_MANUAL_LABEL2
    };
    if (mode == 0) {
        for (int id : randomCtrls) ShowWindow(GetDlgItem(hWnd, id), SW_SHOW);
        for (int id : manualCtrls) ShowWindow(GetDlgItem(hWnd, id), SW_HIDE);
    }
    else {
        for (int id : randomCtrls) ShowWindow(GetDlgItem(hWnd, id), SW_HIDE);
        for (int id : manualCtrls) ShowWindow(GetDlgItem(hWnd, id), SW_SHOW);
    }
    InvalidateRect(hWnd, NULL, TRUE);
    UpdateWindow(hWnd);
}
...
void ResizeControls(HWND hWnd, int cx, int cy) {
    ...
    if (g_currentMode == 0) {
        ...
        if (hVarEdit) SetWindowPos(hVarEdit, NULL, 20, 220, 600 + dx, max(130, 110 + dy), SWP_NOZORDER);
        ...
        if (hInContent) SetWindowPos(hInContent, NULL, 110, 100, 600 + dx, max(140, 110 + dy), SWP_NOZORDER);
    ...
    if (hStatus) SetWindowPos(hStatus, NULL, 20, 625 + dy, 600 + dx, 25, SWP_NOZORDER);
}
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE: {
        ...
        CreateWindowW(L"EDIT", L"", WS_CHILD | WS_VISIBLE | WS_BORDER | ES_MULTILINE | ES_AUTOVSCROLL | WS_VSCROLL,
            20, 220, 600, 130, hWnd, (HMENU)IDC_EDIT_VAR_TEXT, g_hInst, NULL);
        ...
        // C++ 源文件
        CreateWindowW(L"BUTTON", L"浏览...", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 580, 485, 70, 25, hWnd, (HMENU)IDC_BUTTON_BROWSE, g_hInst, NULL);
        CreateWindowW(L"EDIT", L"", WS_CHILD | WS_VISIBLE | WS_BORDER, 20, 485, 550, 25, hWnd, (HMENU)IDC_EDIT_CPP_PATH, g_hInst, NULL);

        // 三个按钮：生成.out、颜色设置、设为默认颜色
        CreateWindowW(L"BUTTON", L"生成 .out", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 20, 535, 150, 35, hWnd, (HMENU)IDC_BUTTON_UNIFIED_COMPILE, g_hInst, NULL);
        CreateWindowW(L"BUTTON", L"颜色设置", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 190, 535, 100, 35, hWnd, (HMENU)IDC_BUTTON_COLOR_SETTING, g_hInst, NULL);
        CreateWindowW(L"BUTTON", L"设为默认颜色", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 310, 535, 120, 35, hWnd, (HMENU)IDC_BUTTON_DEFAULT_COLOR, g_hInst, NULL);

        // g++ 路径
        CreateWindowW(L"STATIC", L"g++ 编译器路径：", WS_CHILD | WS_VISIBLE | SS_RIGHT, 20, 585, 100, 25, hWnd, NULL, g_hInst, NULL);
        CreateWindowW(L"EDIT", L"", WS_CHILD | WS_VISIBLE | WS_BORDER, 125, 585, 400, 25, hWnd, (HMENU)IDC_EDIT_GPP_PATH, g_hInst, NULL);
        CreateWindowW(L"BUTTON", L"浏览...", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 530, 585, 60, 25, hWnd, (HMENU)IDC_BUTTON_GPP_BROWSE, g_hInst, NULL);
        CreateWindowW(L"BUTTON", L"自动检测", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 595, 585, 70, 25, hWnd, (HMENU)IDC_BUTTON_GPP_AUTO, g_hInst, NULL);

        // 状态栏
        CreateWindowW(L"STATIC", L"就绪", WS_CHILD | WS_VISIBLE | SS_SUNKEN, 20, 625, 600, 25, hWnd, (HMENU)IDC_STATIC_STATUS, g_hInst, NULL);
        ...
        for (int id : {IDC_EDIT_MANUAL_INNAME, IDC_EDIT_MANUAL_INCONTENT, IDC_BUTTON_MANUAL_SAVEIN,
            IDC_STATIC_MANUAL_LABEL1, IDC_STATIC_MANUAL_LABEL2}) {
    ...
    case WM_COMMAND: {
        ...
        if (id == IDC_RADIO_MODE_RANDOM) SwitchMode(hWnd, 0);
            ...
            wstring fullInPath = (wstring(inName).find(L'\\') == wstring::npos) ? (g_outputDir + L"\\" + inName) : wstring(inName);
    ...
    case WM_CTLCOLORBTN: {
        ...
        // 如果是复选框（BS_AUTOCHECKBOX），文字颜色跟随用户设置
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE, LPWSTR, int nCmdShow) {
    ...
    g_hMainWnd = CreateWindowExW(WS_EX_CLIENTEDGE, L"CDCDataGenerator", L"测试数据生成器 5.0.0",
        WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, 1000, 760, NULL, NULL, hInstance, NULL);
