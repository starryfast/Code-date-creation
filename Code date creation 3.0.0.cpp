...
#include <cstdlib>     
#include <ctime>
#include <shlobj.h>      // 用于 SHGetFolderPathA
...
void drawMessage(const string& msg) {
	...
	setColor(CYAN);
	cout << msg;
	for (size_t i = msg.length(); i < 60; i++) cout << " ";
	setColor(LIGHT_GRAY);
}
...
string GetDesktopPath() {
	char path[MAX_PATH];
	// SHGetFolderPathA 获取当前用户的桌面目录
	if (SUCCEEDED(SHGetFolderPathA(NULL, CSIDL_DESKTOP, NULL, SHGFP_TYPE_CURRENT, path))) {
		return string(path);
	}
	else {
		// 失败时回退到当前目录
		cerr << "警告：无法获取桌面路径，将使用当前目录。" << endl;
		return ".";
	}
}
...
signed main() {
	...
	cout << "所有数据文件将生成在桌面：" << desktopPath << endl;
	...
	for (int i = a11; i <= b11; ++i) {
		...
		snprintf(fullPath, sizeof(fullPath), "%s\\%d.in", desktopPath.c_str(), i);
	...
	// 生成 .out 文件（空文件，同样保存到桌面）
	for (int i = a11; i <= b11; ++i) {
		char fullPath[MAX_PATH];
		snprintf(fullPath, sizeof(fullPath), "%s\\%d.out", desktopPath.c_str(), i);
		FILE* fp = fopen(fullPath, "w");
		if (fp) fclose(fp);
		else cerr << "错误：无法创建文件 " << fullPath << endl;
	}
	
	cout << "\n所有文件已生成完毕，请到桌面查看。按回车键退出...";
	cin.ignore();
	cin.get();
	return 0;
}
